...
export function DeleteAdminsComponent({ adminId, showDeleteModal, setShowDeleteModal, adminName, setRows }) {
    ...
    const handleSubmit = () => {
        setIsDeleting(true);
        setUploadError("");
        firestore.delete("users/" + adminId)
            .then(res => {
                            ...
                            <>
                                <div className='row mt-3'>
                                    <Typography id="transition-modal-description" style={{ textAlign: "center", color: "#444", top: "-20px" }} className="error-area" sx={{ mt: 2 }}>
                                        Are you sure you want to delete user {adminName}?
                                    </Typography>
                                </div>
